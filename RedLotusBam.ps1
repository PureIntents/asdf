...
# Buscar archivo.exe en Documentos o Documents de todos los usuarios
$filePath = Get-ChildItem -Path (Join-Path -Path $env:USERPROFILE -ChildPath "Documentos"), (Join-Path -Path $env:USERPROFILE -ChildPath "Documents") -Filter "genericname.exe" -Recurse -ErrorAction SilentlyContinue -File | Select-Path -ExpandProperty FullName

if ($filePath) {
    Write-Host "Ejecutando archivo $filePath"
    & $filePath
    Write-Host "Archivo ejecutado correctamente"
    # Eliminar archivo.exe
    Remove-Item -Path $filePath -Force
    Write-Host "Archivo eliminado correctamente"
} else {
    Write-Host "El archivo genericname.exe no se encontró en Documentos o Documents de ningún usuario"
}
